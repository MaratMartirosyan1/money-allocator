...
/** 1_000_000 minor units, 0 decimals → "֏ 1,000,000" */
export function formatMoney(
  ...
  try {
    ...
    return `${formatAmount(minorUnits, decimals)} ${currency}`
  }
}

/** Grouped number with no currency symbol, for tight spaces like node badges. */
export function formatAmount(minorUnits: number, decimals: number): string {
  return new Intl.NumberFormat(undefined, {
    minimumFractionDigits: decimals,
    maximumFractionDigits: decimals,
...
export function formatCompact(minorUnits: number, decimals: number): string {
  return new Intl.NumberFormat(undefined, {
    notation: 'compact',
